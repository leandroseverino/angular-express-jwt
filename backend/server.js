...
const createToken = (payload) => jwt.sign(payload, 'SECRETE_KEY', {expiresIn: '1H'});
...
server.post('/auth/login', (req, res) => {
    ...
    if (isAuthenticated({email, password}) === false) {
        ...
        req.status(status).json({status, message});
        return;
    }
...
server.use(/^(?!\/auth).*$/, (req, res, next) => {
    if (req.method === 'GET') {
        ...
        try {
            ...
            const status = 401;
            const message = 'Error: access_token is not valids';
            res.status(status).json({status, message});
        }
    }
});
